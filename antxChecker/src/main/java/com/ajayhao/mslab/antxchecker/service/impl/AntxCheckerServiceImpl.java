...
public class AntxCheckerServiceImpl implements AntxCheckerService {
    ...
    public List<AntxCheckDetail> checkAntx(String inputPath, String antxContent){
        ...
        if(CollectionUtils.isEmpty(autoConfigs)){
            throw new BusinessBizException(RespCodeType.FILE_NOT_EXIST, "目录下找不到auto-config文件");
        }
    ...
    private List<AntxCheckDetail> parseAutoConfigFile(File autoConfigFile, Set<String> keySet) {
        ...
        try {
            ...
            if(!"config".equals(root.getName())){
                throw new BusinessBizException(RespCodeType.XML_PARSE_ERROR, autoConfigFile.getName(), "找不到config节点");
            }

            List<Element> groups = root.elements();
            if (CollectionUtils.isEmpty(groups)) {
                throw new BusinessBizException(RespCodeType.XML_PARSE_ERROR, autoConfigFile.getName(), "找不到group节点");
            }
            ...
            e.printStackTrace();
            throw new BusinessBizException(RespCodeType.XML_PARSE_ERROR, autoConfigFile.getName(), e.getMessage());
        }
    }
